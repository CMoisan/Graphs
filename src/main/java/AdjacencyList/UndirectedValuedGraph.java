...
public class UndirectedValuedGraph extends UndirectedGraph{
    ...
    public void addEdge(UndirectedNode x, UndirectedNode y, int cost) {
    	if(!isEdge(x,y)){
    		nodes.get(x.getLabel()).addNeigh(nodes.get(y.getLabel()), cost);
    		nodes.get(y.getLabel()).addNeigh(nodes.get(x.getLabel()), cost);
    	}
    }
    ...
    public static void main(String[] args) {
        ...
        al.addEdge(al.getNodes().get(0), al.getNodes().get(2), 6);
        System.out.println(al);
    }
}
