...
public class DirectedGraph extends AbstractListGraph<DirectedNode> implements IDirectedGraph {
    ...
    public void addArc(DirectedNode from, DirectedNode to) {
    	if(!isArc(from,to)) 
    	{
    		from.getSuccs().put(to, to.getLabel());
    	}
    }
    ...
    public IDirectedGraph computeInverse() {
        ...
        for (DirectedNode n : g.getNodes()) {
        	for (DirectedNode nn : nodes) 
        	{
        		if(nn.equals(n)) 
        		{
        			n.setPreds(nn.getSuccs());
        			n.setSuccs(nn.getPreds());
        		}
        	}
        }
        return g;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("isArc Test : " + al.isArc(al.getNodes().get(3), al.getNodes().get(9)));
        al.removeArc(al.getNodes().get(3), al.getNodes().get(9));
        System.out.println(al);
        al.addArc(al.getNodes().get(3), al.getNodes().get(9));
        System.out.println(al);
